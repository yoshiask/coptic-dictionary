...
def sense_list(sense_string):
	...
	for sense in senses:
		...
		if definition is not None:
			sense_html += opener + definition.group(1) + closer
	if len(senses) > 1:
		sense_html += "</ol>"
	return sense_html
...
# 			rows = rows[:100]
			...
			if lemma_exists(word):
				...
				for row in rows:
					...
					tablestring += '<td class="orth_cell">' + '<a href="' + link + '">'
					tablestring += orth
					tablestring += "</a>" + "</td>"
		...
		for row in rows:
			...
			tablestring += '<td class="orth_cell">' + '<a href="' + link + '">' + orth + "</a>" +"</td>"
			tablestring += '<td class="second_orth_cell">' +  second  +"</td>"
...
if __name__ == "__main__":
	...
	params = {}
	word = escape(form.getvalue("coptic", "")).replace("(","").replace(")","").replace("=","").strip()
	dialect = escape(form.getvalue("dialect", "any")).replace("(","").replace(")","").replace("=","").strip()
	pos = escape(form.getvalue("pos", "any")).replace("(","").replace(")","").replace("=","").strip()
	definition = escape(form.getvalue("definition", "")).replace("(","").replace(")","").replace("=","").strip()
	def_search_type = escape(form.getvalue("def_search_type", "exact sequence")).replace("(","").replace(")","").replace("=","")
	def_lang = escape(form.getvalue("lang", "any")).replace("(","").replace(")","").replace("=","").strip()
	related = escape(form.getvalue("related", "false")).replace("(","").replace(")","").replace("=","")
	quick_string = escape(form.getvalue("quick_search", "")).replace("(","").replace(")","").replace("=","").strip()
	page = 1
	page = escape(form.getvalue("page", "1")).replace("(","").replace(")","").replace("=","").strip()
	params["coptic"] = word
...
"""
	...
	if word != "": # nothing to do if no coptic word searched?
		if related == "false":
			...
			#results_page += '<a href="' + link + '">Include related entries</a>'
			if not "window.location" in results_page:  # No need to retrieve related if we are redirecting due to a unique entry being found
				results_page = results_page[:-8] + '<a href="' + link + '">Include related entries</a></div>\n'
		elif related == "true":
			if not "window.location" in results_page:  # No need to retrieve related if we are redirecting due to a unique entry being found
