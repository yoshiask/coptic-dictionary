...
def get_annis_query(coptic, oref, cs_pos=None):
    ...
    if " " in coptic:
        ...
        query = "_q=" + urlsafe_b64encode(('norm_group=/.*' + coptic + '.*/').encode('utf-8')).decode('utf-8')
    elif " " in oref:
        ...
        query = "_q=" + urlsafe_b64encode(query.encode('utf-8')).decode('utf-8')
    elif cs_pos in ["VSTAT","VIMP"]: # This is an inflected entry, look for norm and pos
        query = "_q=" + urlsafe_b64encode(('norm="'+ coptic + '" _=_ pos="'+str(cs_pos)+'"').encode('utf-8')).decode('utf-8')
    else:
        query = "_q=" + urlsafe_b64encode(('lemma="'+ coptic + '"').encode('utf-8')).decode('utf-8')
...
def get_annis_entity_query(coptic, entity_type):
    ...
    query = "_q=" + urlsafe_b64encode(q.encode('utf-8')).decode('utf-8')
